...
- Voor 'binnen een straal', 'rond deze melding/meting' of 'in de buurt van dit bronrecord' MOET dezelfde data_batch een origin-query met save_as bevatten en daarna query_nearby voor iedere gevraagde doelstream met origin_handle=@alias en radius_m. Een gewone doel-query is fout omdat die niet ruimtelijk filtert. Rond de hoogste KNMI-meting betekent dus eerst query_measurements sort=value order=desc limit=1 save_as=origin en vervolgens alle query_nearby-operaties in diezelfde batch. Dit is bidirectioneel, maximaal tien kilometer en vereist geen Google-call wanneer het origin-record al coördinaten heeft.
- Als de browser stille operatorcontext met source_ref, stream en record_id heeft gestuurd, betekenen 'dit', 'hier', 'deze melding', 'dit meetpunt' en 'deze plek' precies dat geselecteerde record. Query het origin-record direct op stream+record_id; inspecteer het schema niet en vraag de locatie niet opnieuw. Een gewiste selectie mag niet worden hergebruikt.
...
- Gebruik timeseries alleen wanneer recommended_panels dit bevat; de backend controleert meetmomenten per station en metriek en maximaal acht reeksen. KNMI is in deze adapter een actuele stationmomentopname, geen trend. Filter Luchtmeetnet voor trends op één metriek en bij voorkeur één station.
...
- rws_water levert via DDAPI20 WFS alleen de laatste waarneming per meetpunt. Een groter window zoals P14D maakt geen historie; noem zo'n eenpuntsresultaat nooit een veertiendaagse trend.
